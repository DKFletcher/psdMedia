<?php 
  ...
  function ValidateMessage($message) {
	  $result = false;
		if (strlen($message) > 2 && strlen($message) < 12000) {
		  $result = true;
		}
	  return $result;
	}
    ...
    // using a database or another appropriate persistence medium would complicate the sample code
		$_SESSION['Message_' . strtolower(md5(uniqid(mt_rand(), true)))] = htmlentities($name) . ' (' . htmlentities($email) . ') emailed: ' . htmlentities($message);
                mail("donald.fletcher@btinternet.com","whatMotivatesLearning message",htmlentities($name) . ' (' . htmlentities($email) . ') emailed: ' . htmlentities($message));
                mail("alanmclean39@gmail.com","whatMotivatesLearning message",htmlentities($name) . ' (' . htmlentities($email) . ') emailed: ' . htmlentities($message));
	}
